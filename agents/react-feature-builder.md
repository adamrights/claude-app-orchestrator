...
1. **Enumerate the existing critical paths** the blueprint's `features[].description` (on prior features) and `pages:` list. Derive them from THIS blueprint — e.g., for a support-ticketing app they'd be the ticket creation flow, detail + status transitions, role-gated threads, RBAC enforcement, and admin user management. Never reuse another app's list.
